# VK Career Text‑to‑Image

Генерация карьерных иллюстраций во вселенной VK на основе Stable Diffusion XL и Gradio.

## О проекте

Этот репозиторий содержит учебный проект в рамках VK Education: веб‑интерфейс, который помогает студентам и начинающим разработчикам быстро получать иллюстрации для самопрезентаций, описаний достижений и командной работы во стиле VK.

Под капотом используется модель **Stable Diffusion XL 1.0** через библиотеку **diffusers**, отдельный сервисный слой с промпт‑инжинирингом и интерфейс на **Gradio**.

## Возможности

- Три преднастроенные сцены:
  - «Самопрезентация»
  - «Профессиональное достижение»
  - «Команда VK»
  - плюс режим «Свободный текст» без ограничений по содержанию.
- Два визуальных стиля:
  - `vk_flat` — минималистичная flat‑иллюстрация в мягких фирменных сине‑фиолетовых тонах VK.
  - `neon` — тёмный фон с неоновыми сине‑фиолетовыми акцентами и кинематографичным светом.
- Автоматический перевод пользовательского текста с русского на английский (`deep-translator`), чтобы стабильно работать с SDXL.
- Negative prompt для карьерных сцен (отсечение животных, лишнего текста и артефактов) и свободный текст без ограничений.
- Веб‑интерфейс на Gradio:
  - выбор сцены и стиля;
  - ввод текста на русском или английском;
  - «Расширенные настройки» (число шагов и guidance scale);
  - блок примеров карьерных запросов.

## Примеры генераций (стили vk_flat/neon)

Файлы лежат в папке `examples/`.

Самопрезентация:

![Самопрезентация, vk_flat](examples/Самопрезентация_vk_flat.jpg)  
![Самопрезентация, neon](examples/Самопрезентация_neon.jpg)

Профессиональное достижение:

![Профессиональное достижение, vk_flat](examples/Профессиональное_достижение_vk_flat.jpg)  
![Профессиональное достижение, neon](examples/Профессиональное_достижение_neon.jpg)

Команда VK:

![Команда VK, vk_flat](examples/Команда_VK_vk_flat.jpg)  
![Команда VK, neon](examples/Команда_VK_neon.jpg)

Свободный текст (пример пользовательского запроса `кот смотрит телевизор`):

![Свободный текст, neon](examples/Кот_смотрит_телевизор_VK_flat.jpg)
![Свободный текст, neon](examples/Кот_смотрит_телевизор_neon.jpg)

## Архитектура

- `service_layer.py` — инициализация Stable Diffusion XL, базовые промпты сцен, стили `vk_flat` / `neon`, перевод RU→EN и negative prompts.  
- `05_gradio_app.ipynb` — Gradio‑интерфейс с выбором сцены, стиля, вводом текста, настройками качества и блоком примеров.

Сервисный слой отделён от интерфейса, поэтому при желании его можно переиспользовать в другом приложении или вынести в отдельный сервис.

## Как запустить

### В Google Colab

1. Открыть ноутбук `05_gradio_app.ipynb` из репозитория в Colab.  
2. В меню «Среда выполнения» выбрать GPU (T4/L4/A100) или при отсутствии — CPU.  
3. Выполнить ячейки по порядку:
   - установка зависимостей (если есть соответствующая ячейка);
   - импорт и перезагрузка `service_layer`;
   - финальная ячейка с кодом Gradio и `demo.launch()`.  
4. Открыть ссылку на Gradio‑интерфейс в выводе и поэкспериментировать со сценами и стилями.

### Локально

1. Клонировать репозиторий:
```
git clone https://github.com/desve/vk-text-to-image-demo.git
cd vk-text-to-image-demo
```

2. Установить зависимости:
```
pip install -r requirements.txt
```

3. Запустить приложение одним из способов:
   - открыть `05_gradio_app.ipynb` в Jupyter / VS Code и выполнить ячейки;  
   - или вынести код блока Gradio в `app.py` и запустить:
```
python app.py
```

## Ограничения и возможное развитие

Ограничения:

- SDXL иногда даёт неточные детали (особенно руки, мелкие объекты и текст).  
- Генерация на CPU заметно медленнее; для комфортной работы желательно GPU с 8–10 ГБ VRAM.  
- Переводчик может слегка переформулировать сложные фразы.

Идеи развития:

- выкатить демо в Hugging Face Spaces как публичный Gradio‑Space;  
- добавить сохранение и галерею лучших кадров с подписями;  
- вынести сервисный слой в отдельный модуль или микросервис;  
- дообучить модель или подключить LoRA под фирменный визуальный стиль VK.

## Автор

Автор: GitHub‑профиль https://github.com/desve

Репозиторий создан в рамках образовательных проектов VK Education и используется как часть портфолио для дальнейшего развития в области генеративного ИИ и промпт‑инжиниринга: проект демонстрирует работу с SDXL, сценовую модель промптов, negative prompts и пользовательский интерфейс для исследований визуальных стилей.


